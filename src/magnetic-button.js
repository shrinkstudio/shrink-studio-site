...
export function initMagneticButtons(scope) {
  scope = scope || document;
  var magnets = scope.querySelectorAll("[data-magnetic-strength]");
  ...
  magnets.forEach(function (m) {
    ...
    listeners.push(
      { element: m, type: "mouseenter", handler: resetOnEnter },
      { element: m, type: "mousemove", handler: moveMagnet },
      { element: m, type: "mouseleave", handler: resetMagnet }
    );
  });
}

export function destroyMagneticButtons() {
  listeners.forEach(({ element, type, handler }) => {
    element.removeEventListener(type, handler);
    gsap.killTweensOf(element);
    var inner = element.querySelector("[data-magnetic-inner-target]");
